...
class BOCSpider(BaseSpider):
    """中国银行信用卡活动爬虫"""
    
    def __init__(self):
        super().__init__()
        self.name = "中国银行"
        self.base_url = "https://www.bankofchina.com"
        self.activity_url = "https://www.bankofchina.com/pbservice/bi3/"
    ...
    async def crawl(self) -> List[Dict]:
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0, follow_redirects=True) as client:
                ...
                logger.info(f"中国银行爬取成功，获取 {len(activities)} 条活动")
        except Exception as e:
            logger.error(f"中国银行爬取失败: {e}")
    ...
    def parse_activity(self, html_content: str) -> List[Dict]:
        ...
        for item in activity_items[:20]:
            try:
                ...
                logger.warning(f"解析中国银行活动项失败: {e}")
