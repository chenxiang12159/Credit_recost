...
class ICBCSpider(BaseSpider):
    ...
    def __init__(self):
        ...
        self.activity_url = "https://www.icbc.com.cn/page/1210271808395468800.html"
    ...
    async def crawl(self) -> List[Dict]:
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0, follow_redirects=True) as client:
                ...
                logger.info(f"工商银行爬取成功，获取 {len(activities)} 条活动")
        except Exception as e:
            logger.error(f"工商银行爬取失败: {e}")
    ...
    def parse_activity(self, html_content: str) -> List[Dict]:
        ...
        # 工行活动页面结构
        activity_items = soup.find_all(['div', 'li'], class_=re.compile(r'(item|list|activity)', re.I))
        ...
        for item in activity_items[:20]:
            try:
                ...
                logger.warning(f"解析工商银行活动项失败: {e}")
