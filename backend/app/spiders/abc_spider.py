...
class ABCSpider(BaseSpider):
    """农业银行信用卡活动爬虫"""
    
    def __init__(self):
        super().__init__()
        self.name = "中国农业银行"
        self.base_url = "https://www.abchina.com"
        self.activity_url = "https://www.abchina.com/cn/PersonalServices/ABCPromotion/National/yykh20251208.htm"
    ...
    async def crawl(self) -> List[Dict]:
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0, follow_redirects=True) as client:
                ...
                logger.info(f"农业银行爬取成功，获取 {len(activities)} 条活动")
        except Exception as e:
            logger.error(f"农业银行爬取失败: {e}")
    ...
    def parse_activity(self, html_content: str) -> List[Dict]:
        ...
        # 农行活动通常在列表中
        activity_items = soup.find_all(['li', 'div'], class_=re.compile(r'(activity|promo|item)', re.I))
        
        for item in activity_items[:20]:  # 限制数量
            try:
                title_elem = item.find(['a', 'span', 'h3'])
                if not title_elem:
                    continue
                ...
                logger.warning(f"解析农业银行活动项失败: {e}")
